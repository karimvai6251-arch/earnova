...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "home":
        ...
        await q.edit_message_text(f"💰 Balance: *{u[2]:.2f} ৳*", parse_mode="Markdown", reply_markup=menu())
    elif q.data == "profile":
        await q.edit_message_text(
            f"👤 *Profile*\n\nID: `{uid}`\nBalance: *{u[2]:.2f} ৳*\n"
            f"Referrals: *{u[4]}*\nBF Level: *{u[5]}*",
        ...
        await q.edit_message_text("📝 *Available Tasks*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
        ...
        kb.append([InlineKeyboardButton("🔙 Back", callback_data="home")])
        await q.edit_message_text("🚀 *BF / VIP Packages*\nChoose a package:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
        ...
        bal = cur.fetchone()[0]
        if bal < price:
            await q.edit_message_text(f"❌ Balance কম।\nPrice: {price:.0f} ৳\nYour balance: {bal:.2f} ৳", reply_markup=menu())
        else:
            cur.execute("UPDATE users SET balance=balance-?, bf_level=? WHERE user_id=?", (price,level,uid))
            conn.commit()
            await q.edit_message_text(f"🎉 BF {level} activated!\nExtra earning rate: +{rate}%", reply_markup=menu())
        ...
        await q.edit_message_text(
            ...
            "একটি payment method বেছে নাও:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
    elif q.data.startswith("wd:"):
        method = q.data[3:]
        context.user_data["withdraw_method"] = method
        await q.edit_message_text(
            f"💸 {method}\n\nএখন amount এবং account/number পাঠাও এভাবে:\n"
            "`500 01XXXXXXXXX`\n\nBinance হলে UID/Pay ID দিতে পারো।", parse_mode="Markdown")
    elif q.data == "stats":
        cur.execute("SELECT COUNT(*), COALESCE(SUM(balance),0) FROM users")
        c, b = cur.fetchone()
        await q.edit_message_text(f"📊 *Statistics*\n\n👥 Total Users: {c}\n💰 Total Distributed Balance: {b:.2f} ৳", parse_mode="Markdown", reply_markup=menu())
    elif q.data == "help":
        await q.edit_message_text(
            "ℹ️ *Help*\n\n"
            "📝 Tasks → task করে আয়\n"
    ...
    if uid == ADMIN_ID:
        if q.data == "adminhome":
            ...
            await q.edit_message_text(
                ...
                "বাতিল: /cancel", parse_mode="Markdown", reply_markup=admin_kb())
            ...
            if not rows:
                ...
                kb = []
                for r in rows:
                    text += f"#{r[0]} | User: {r[1]} | {r[2]:.2f}৳ | {r[3]} | {r[4]}\n"
            ...
            conn.commit()
            await q.edit_message_text(f"✅ Withdrawal #{wid} approved.\nম্যানুয়ালি payment পাঠিয়ে দাও।", reply_markup=admin_kb())
            ...
            if r:
                ...
                conn.commit()
            await q.edit_message_text(f"❌ Withdrawal #{wid} rejected and balance refunded.", reply_markup=admin_kb())
        elif q.data == "a_balance":
            ...
            await q.edit_message_text("💳 নতুন Minimum Withdraw amount পাঠাও।\nউদাহরণ: `100`\n\n/cancel", parse_mode="Markdown", reply_markup=admin_kb())
        elif q.data == "set_ref":
            context.user_data["admin_input"] = "ref"
            await q.edit_message_text("👥 নতুন Referral Reward পাঠাও।\nউদাহরণ: `10`\n\n/cancel", parse_mode="Markdown", reply_markup=admin_kb())
        elif q.data == "set_task":
            context.user_data["admin_input"] = "task"
            await q.edit_message_text(
                "📝 Task এভাবে পাঠাও:\n`Task Name | Reward | URL`\n\nউদাহরণ:\n`Join Channel | 5 | https://t.me/example`\n\n/cancel",
                parse_mode="Markdown", reply_markup=admin_kb())
        elif q.data == "set_tasks":
            ...
            if not rows:
                ...
                for tid, title, reward, active in rows:
                    ...
                    kb.append([
                        InlineKeyboardButton(f"{'🔴 OFF' if active else '🟢 ON'} #{tid}", callback_data=f"toggle_task:{tid}"),
                ...
                kb.append([InlineKeyboardButton("🔙 Admin Panel", callback_data="adminhome")])
                await q.edit_message_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
        elif q.data.startswith("delete_task:"):
            tid = int(q.data.split(":")[1])
            cur.execute("DELETE FROM tasks WHERE id=?", (tid,))
            conn.commit()
            await q.edit_message_text("🗑️ Task deleted successfully.", reply_markup=admin_kb())
        elif q.data.startswith("toggle_task:"):
            tid = int(q.data.split(":")[1])
...
async def text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == ADMIN_ID:
        ...
        if action == "methods":
            ...
            context.user_data.pop("admin_input", None)
            await update.message.reply_text("✅ Payment methods updated.", reply_markup=admin_kb())
            return
        ...
        if action == "balance":
            try:
                ...
                conn.commit()
                context.user_data.pop("admin_input", None)
                await update.message.reply_text(f"✅ User {target} balance updated by {amount:g} ৳", reply_markup=admin_kb())
            except Exception:
        ...
        if action == "min":
            try:
                ...
                await update.message.reply_text("❌ শুধু সঠিক সংখ্যা দাও।")
            return
        ...
        if action == "task":
            ...
            try:
                ...
                cur.execute("INSERT INTO tasks(title,reward,url,active) VALUES(?,?,?,1)", (parts[0], reward, parts[2]))
                conn.commit()
                context.user_data.pop("admin_input", None)
        ...
        if context.user_data.pop("broadcast", False):
            ...
            for x in ids:
                try:
                    ...
                    pass
            await update.message.reply_text(f"📢 Broadcast sent to {sent} users.", reply_markup=admin_kb())
            return
    ...
    if method:
        ...
        cur.execute("""INSERT INTO withdrawals(user_id,amount,method,account,created_at)
                       VALUES(?,?,?,?,?)""", (uid, amount, method, account, datetime.utcnow().isoformat()))
        conn.commit()
        context.user_data.pop("withdraw_method", None)
        await update.message.reply_text("✅ Withdraw request submitted.\nAdmin review করবে।", reply_markup=menu())
        try:
