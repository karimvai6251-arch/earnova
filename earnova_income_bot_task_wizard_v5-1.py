...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💸 Withdraw Requests", callback_data="apending")],
        [InlineKeyboardButton("💳 Payment Methods", callback_data="payment_methods")],
        [InlineKeyboardButton("📝 Task Manager", callback_data="set_tasks"),
         InlineKeyboardButton("➕ Add Task", callback_data="set_task")],
        [InlineKeyboardButton("💰 Balance Control", callback_data="a_balance"),
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "balance":
        ...
        raw = setting("payment_methods") or "bKash, Nagad, Rocket, Binance, Mobile Recharge"
        methods = [x.strip() for x in raw.split(",") if x.strip()]
        kb = [[InlineKeyboardButton(x, callback_data=f"wd:{x}")] for x in methods]
        kb.append([InlineKeyboardButton("🔙 Back", callback_data="home")])
        await q.edit_message_text(
            f"💸 *Withdraw*\nMinimum: {setting('min_withdraw')} ৳\n\n"
            "একটি payment method বেছে নাও:",
            parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
    ...
    # Admin callbacks
    if uid == ADMIN_ID and q.data == "payment_methods":
        methods = setting("payment_methods") or "bKash, Nagad, Rocket, Binance, Mobile Recharge"
        await q.edit_message_text(
            f"💳 *PAYMENT METHODS*\n\nবর্তমানে চালু:\n{methods}\n\n"
            "Payment method পরিবর্তন করতে নিচের button চাপো।",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✏️ Edit Methods", callback_data="edit_methods")],
                [InlineKeyboardButton("🔙 Admin Panel", callback_data="adminhome")]
            ]))
        return

    if uid == ADMIN_ID and q.data == "edit_methods":
        context.user_data["admin_input"] = "methods"
        await q.edit_message_text(
            "💳 কোন কোন payment method রাখবে তা এক লাইনে লিখো।\n\n"
            "উদাহরণ:\n`bKash, Nagad, Rocket, Binance, Mobile Recharge`\n\n"
            "যে method লিখবে, user-এর Withdraw menu-তে সেটাই দেখাবে।\n"
            "বাতিল: /cancel",
    ...
    if uid == ADMIN_ID and q.data == "adminhome":
        ...
        await q.edit_message_text(f"❌ Withdrawal #{wid} rejected and balance refunded.",
                                  reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "a_balance":
        context.user_data["admin_input"] = "balance"
        await q.edit_message_text(
            "💰 *Balance Control*\n\n"
            "এই format-এ পাঠাও:\n`USER_ID | AMOUNT`\n\n"
            "উদাহরণ: `123456789 | 50`\n"
            "৫০ যোগ হবে। কমাতে `123456789 | -50`।\n\n"
            "বাতিল: /cancel",
            parse_mode="Markdown", reply_markup=admin_kb())
        ...
        await q.edit_message_text(
            ...
            parse_mode="Markdown", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_link_ok":
        context.user_data["task_wizard"]["step"] = "name"
        await q.edit_message_text(
            "✍️ *Step 2/4 — Task Name*\n\n"
            "এখন Task-এর নাম লিখো।\nউদাহরণ: `Join Telegram Channel`",
            parse_mode="Markdown", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_link_retry":
        context.user_data["task_wizard"]["step"] = "link"
        await q.edit_message_text(
            "🔗 আবার Task-এর সঠিক Link পাঠাও।",
            reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_confirm":
        w = context.user_data.get("task_wizard", {})
        if not all(k in w for k in ("link", "name", "reward")):
            await q.edit_message_text("❌ Task information incomplete.", reply_markup=admin_kb())
            return
        try:
            reward = float(w["reward"])
            cur.execute("INSERT INTO tasks(title,reward,url,active) VALUES(?,?,?,1)",
                        (w["name"], reward, w["link"]))
            conn.commit()
            ...
            await q.edit_message_text(
                ...
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➕ Add Another Task", callback_data="set_task")],
                    [InlineKeyboardButton("📋 Task Manager", callback_data="set_tasks")],
                    [InlineKeyboardButton("🔙 Admin Panel", callback_data="adminhome")]
                ]))
        except Exception:
            await q.edit_message_text("❌ Task save হয়নি। আবার চেষ্টা করো।", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_cancel":
        context.user_data.pop("task_wizard", None)
        await q.edit_message_text("❌ Task creation cancelled.", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data.startswith("task_reward:"):
        wizard = context.user_data.get("task_wizard")
        if not wizard or wizard.get("step") != "reward":
            await q.edit_message_text("❌ Task wizard expired.", reply_markup=admin_kb())
            return
        amount = float(q.data.split(":")[1])
        wizard["reward"] = amount
        wizard["step"] = "confirm"
        context.user_data["task_wizard"] = wizard
        await q.edit_message_text(
            "🔎 *Step 4/4 — Confirm Task*\n\n"
            f"🔗 Link: {wizard['link']}\n"
            f"📝 Name: {wizard['name']}\n"
            f"💰 Income: {amount:g} ৳\n\n"
            "সবকিছু ঠিক আছে?",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Add Task", callback_data="task_confirm")],
                [InlineKeyboardButton("✏️ Change Income", callback_data="task_reward_custom")],
                [InlineKeyboardButton("✏️ Change Name", callback_data="task_change_name")],
                [InlineKeyboardButton("🔗 Change Link", callback_data="task_change_link")],
                [InlineKeyboardButton("❌ Cancel", callback_data="task_cancel")]
            ]))
    elif uid == ADMIN_ID and q.data == "task_reward_custom":
        wizard = context.user_data.get("task_wizard", {})
        wizard["step"] = "reward_custom"
        context.user_data["task_wizard"] = wizard
        await q.edit_message_text(
            "💰 Custom Income লিখো।\nউদাহরণ: `7.5`",
            parse_mode="Markdown", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_change_name":
        wizard = context.user_data.get("task_wizard", {})
        wizard["step"] = "name"
        context.user_data["task_wizard"] = wizard
        await q.edit_message_text("✍️ নতুন Task Name লিখো।", reply_markup=admin_kb())
    elif uid == ADMIN_ID and q.data == "task_change_link":
        wizard = context.user_data.get("task_wizard", {})
        wizard["step"] = "link"
        context.user_data["task_wizard"] = wizard
        await q.edit_message_text("🔗 নতুন Task Link পাঠাও।", reply_markup=admin_kb())
        ...
        if not rows:
            ...
            for tid, title, reward, active in rows:
                ...
                kb.append([
                    InlineKeyboardButton(f"{'🔴 OFF' if active else '🟢 ON'} #{tid}",
                                         callback_data=f"toggle_task:{tid}"),
                    InlineKeyboardButton("✏️ Edit", callback_data=f"edit_task:{tid}"),
                    InlineKeyboardButton("🗑️ Delete", callback_data=f"delete_task:{tid}")
                ])
            kb.append([InlineKeyboardButton("➕ Add Task", callback_data="set_task")])
            kb.append([InlineKeyboardButton("🔙 Admin Panel", callback_data="adminhome")])
            await q.edit_message_text(text, parse_mode="Markdown",
                                      reply_markup=InlineKeyboardMarkup(kb))
    elif uid == ADMIN_ID and q.data.startswith("delete_task:"):
        tid = int(q.data.split(":")[1])
        cur.execute("SELECT title FROM tasks WHERE id=?", (tid,))
        row = cur.fetchone()
        if not row:
            await q.edit_message_text("❌ Task পাওয়া যায়নি।", reply_markup=admin_kb())
        else:
            cur.execute("DELETE FROM tasks WHERE id=?", (tid,))
            conn.commit()
            await q.edit_message_text(
                f"🗑️ Task deleted: {row[0]}",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📋 Task Manager", callback_data="set_tasks")],
                    [InlineKeyboardButton("🔙 Admin Panel", callback_data="adminhome")]
                ]))
    elif uid == ADMIN_ID and q.data.startswith("edit_task:"):
        tid = int(q.data.split(":")[1])
        cur.execute("SELECT title,reward,url FROM tasks WHERE id=?", (tid,))
        row = cur.fetchone()
        if not row:
            await q.edit_message_text("❌ Task পাওয়া যায়নি।", reply_markup=admin_kb())
        else:
            context.user_data["admin_input"] = f"edit_task:{tid}"
            await q.edit_message_text(
                f"✏️ *Edit Task #{tid}*\n\nবর্তমান:\n"
                f"Name: {row[0]}\nReward: {row[1]}৳\nLink: {row[2]}\n\n"
                "নতুন তথ্য এভাবে পাঠাও:\n"
...
async def text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == ADMIN_ID:
        ...
        if wizard:
            ...
            if step == "link":
                if not (text.startswith("http://") or text.startswith("https://") or text.startswith("tg://")):
                    await update.message.reply_text(
                        "❌ Link সঠিক নয়। http:// বা https:// দিয়ে শুরু হওয়া link দাও।")
                    return
                wizard["link"] = text
                context.user_data["task_wizard"] = wizard
                await update.message.reply_text(
                    "🔗 Link received:\n"
                    f"{text}\n\n"
                    "এখন Link ঠিক আছে কিনা নিশ্চিত করো:",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("✅ Link ঠিক আছে", callback_data="task_link_ok")],
                        [InlineKeyboardButton("🔄 Link আবার দেব", callback_data="task_link_retry")],
                        [InlineKeyboardButton("❌ Cancel", callback_data="task_cancel")]
                    ]))
                return
            if step == "name":
                if len(text) < 2:
                    await update.message.reply_text("❌ Task name একটু বড় করে দাও।")
                    return
                wizard["name"] = text
                wizard["step"] = "reward"
                context.user_data["task_wizard"] = wizard
                await update.message.reply_text(
                    "💰 *Step 3/4 — Task Income*\n\n"
                    "কত টাকা Income/Reward হবে সেটা নিচের button থেকে বেছে নাও:",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("৳1", callback_data="task_reward:1"),
                         InlineKeyboardButton("৳2", callback_data="task_reward:2"),
                         InlineKeyboardButton("৳5", callback_data="task_reward:5")],
                        [InlineKeyboardButton("৳10", callback_data="task_reward:10"),
                         InlineKeyboardButton("৳20", callback_data="task_reward:20"),
                         InlineKeyboardButton("৳50", callback_data="task_reward:50")],
                        [InlineKeyboardButton("✍️ Custom Amount", callback_data="task_reward_custom")],
                        [InlineKeyboardButton("❌ Cancel", callback_data="task_cancel")]
                    ]))
                return
            if step == "reward_custom":
                try:
                    amount = float(text)
                    if amount <= 0:
                        raise ValueError
                    ...
                    await update.message.reply_text("❌ সঠিক amount দাও।")
                return
        ...
        if action == "methods":
            ...
            if not methods:
                await update.message.reply_text("❌ অন্তত ১টি payment method দাও।")
                return
            set_setting("payment_methods", ", ".join(methods))
            context.user_data.pop("admin_input", None)
            await update.message.reply_text(
        ...
        if action == "balance":
            try:
                uid_text, amount_text = [x.strip() for x in text.split("|", 1)]
                target = int(uid_text)
                ...
                context.user_data.pop("admin_input", None)
                await update.message.reply_text(
                    f"✅ User {target} balance {'+' if amount >= 0 else ''}{amount:g} ৳",
                    reply_markup=admin_kb())
            except Exception:
        ...
        if action == "ref":
            ...
            return
        if action and action.startswith("edit_task:"):
            ...
            try:
                ...
                cur.execute("UPDATE tasks SET title=?, reward=?, url=? WHERE id=?",
                            (parts[0], reward, parts[2], tid))
                conn.commit()
                context.user_data.pop("admin_input", None)
                await update.message.reply_text(
                    f"✅ Task #{tid} updated successfully.", reply_markup=admin_kb())
            except Exception:
